    ...
    <div class="consultation-container">
                ...
                <div class="warm-tips">
                    ...
                    <div class="healing-actions" 
                        ...
                        <div class="actions-list">
                            <div v-for="action in currentEmotion.improvementSuggestions" 
                                :key="action" class="action-item">
                            ...
                            <div class="session-title">
                                ...
                                <div class="session-stats">
                                    ...
                                    <span class="message-count">
                                        ...
                                        {{ formatDuration(session.durationMinutes) || '0分钟'  }}
                                    </span>
                                </div>
                            </div>
        ...
        <div class="chat-main" ref="chatMainEl">
                ...
                <div class="header-left">
                    <div class="chat-avatar">
                        <el-image :src="iconUrl1"
                            style="width:24px;height:24px" alt="AI助手" />
                    </div>
                    <div class="chat-info">
                        <h2>宁渡AI助手</h2>
                        <p>您贴心的AI心理健康助手</p>
            ...
            <div class="chat-messages">
                ...
                <!-- 展示消息列表 -->
                <div v-for="msg in sumMessage" :key="msg.id" class="message-item"
                    :class="msg.senderType===1?'user-message':'ai-message'">
                    <div class="message-avatar">
                        <el-image v-if="msg.senderType===1" :src="iconUrl2"
                            style="width:18px;height:18px"/>
                        <el-image v-else-if="msg.senderType===2" :src="iconUrl"
                            style="width:18px;height:18px;"/>
                    </div>
                    <div class="message-content">
                        <div class="message-bubble">
            ...
            <!-- 消息输入区域 -->
             <div class="chat-input">
                ...
                <el-button :disabled="!userMessage.trim() || userMessage.length>500" 
                    type="primary" class="send-btn" @click="sendMessage">
...
//以左侧侧边栏高度为基准同步右侧聊天区高度，消息多时内部滚动
const syncChatHeight=()=>{
    if(sidebarEl.value&&chatMainEl.value){
        const h=sidebarEl.value.offsetHeight
        if(h>0) chatMainEl.value.style.height=h+'px'
    }
}
...
const currentEmotion=ref({
    ...
    riskDescription:'您的情绪状态为负面，风险等级为3，建议您及时处理。'
})
...
const startNewSession=(message)=>{
    ...
    startSession(sessionParams).then(res=>{
        ...
        const sessionData={
            ...
            status:res.status,
            sessionTime:sessionParams.sessionTitle
        }
        //当前是临时会话
...
const startAIResponse=(sessionId,userMessage)=>{
    ...
    sumMessage.value.push(aiMessage)
    // 调用流式接口
    //这个是专门终止fetch请求的
    ...
    fetchEventSource(`/api/psychological-chat/stream`,{
        ...
        onmessage:(event)=>{
            ...
            //获取当前会话的AI消息
            const aiMessage=sumMessage.value[sumMessage.value.length-1]
...
const getSessionPage=()=>{
    getSessionList({
        ...
        console.log("完整session：",res.records?.[0])
...
const getIntensityClass=(score)=>{
    if(score<0 ||score>100){
        ElMessage.error('情绪强度不在0-100之间,情绪输入异常!请反馈给项目组!')
        throw new Error('情绪强度不在0-100之间,情绪输入异常!请反馈给项目组!')
    }
    if(score>=61){
        return 3
    }else if(score>=31){
...
onMounted(()=>{
    ...
    nextTick(()=>{
        syncChatHeight()
        if(window.ResizeObserver&&sidebarEl.value){
            chatHeightObserver=new ResizeObserver(syncChatHeight)
            chatHeightObserver.observe(sidebarEl.value)
        }
    })
})
onBeforeUnmount(()=>{
    if(chatHeightObserver){chatHeightObserver.disconnect();chatHeightObserver=null}
})
</script>
